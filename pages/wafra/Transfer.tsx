...
const TransferPage = () => {
  ...
  const IToken = {
    ...
    EditForm: (): DataTableEditForm => {
      return {
        ...
        formComponents: {
          ...
          children: [
            { objectName: "senderUserCode", objectType: "Input", props: ({ id: "senderUserCode", label: "senderUserCode", onValid: [(that) => { return { IsValid: (that.getValue() != null && that.getValue().length > 0), ValidText: "Değer Giriniz" } }] } as InputProps) },
            { objectName: "receiverUserCode", objectType: "Input", props: ({ id: "receiverUserCode", label: "receiverUserCode", onValid: [(that) => { return { IsValid: (that.getValue() != null && that.getValue().length > 0), ValidText: "Değer Giriniz" } }] } as InputProps) },
            { objectName: "tokenQuantity", objectType: "Input", props: ({ id: "tokenQuantity", label: "tokenQuantity", onValid: [(that) => { return { IsValid: (that.getValue() != null && that.getValue().length > 0), ValidText: "Değer Giriniz" } }] } as InputProps) },
    ...
    DeleteDatas: async () => {
      ...
      if (deleteProcess.isSuccess == false) {
        ...
        Nar.Modal.Alert("IslemBasarili", "TokenTransferIptalEdildi", "Ok");
      }
  ...
  return <>
    <useScreen.View screencode="TokenTransfer" screenname="Token Aktarım İşini Yapar" saveType={SaveType.disable} onSaveClick={IToken.onSaveClick}>
      <table.View
        id="TokenTransfer"
        maxHeight={800}
        ...
        columns={
          [
            { dataKey: "senderUserCode", columnName: "senderUserCode", columnControllerType: ControllerType.Input, isNotRemoteFilter: true, isNotEdit: true },
            { dataKey: "receiverUserCode", columnName: "receiverUserCode", columnControllerType: ControllerType.Input, isNotRemoteFilter: true },
            { dataKey: "tokenQuantity", columnName: "tokenQuantity", columnControllerType: ControllerType.Input, isNotRemoteFilter: true, isNotEdit: true },
